...
class Mem_map_APB_Trans extends APB_Trans;
  ...
  typedef enum {ROM, RAM, IO} area_t;
  rand area_t area;
...
class Mem_map_APB_Trans_ascending extends Mem_map_APB_Trans;

  // Constrains RAM write transactions to have an ascending address

  T_addr previous_addr = 'h8000;
  ...
  function void post_randomize;
    if ((dir == dir_Wr) && (area == RAM))
    begin
      addr = previous_addr + $urandom_range(1, 'hf);
      previous_addr = addr;
    end
  endfunction
  
endclass : Mem_map_APB_Trans_ascending
