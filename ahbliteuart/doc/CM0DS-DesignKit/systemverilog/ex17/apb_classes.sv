...
class APB_Stim;
  ...
  // TO DO:   Complete the body() method of this class.  It should
  ...
  task body();
    ...
    repeat (num_trans)
    begin
      // TO DO:  Randomise the object, send a copy of it
      //         to the sink channel

        assert( template.randomize());
        $cast(tr, template.copy());
        sink.put(tr);
...
class APB_Driver;
  ...
  // TO DO:  Complete the body() method of this class.
  //
  task body();
    // TO DO:  Any necessary variable declarations.
    APB_Trans current;
    Transaction t;
  ...
  //$cast(current,t);
    do
      drive(current);
    while (source.try_get(current));


    // TO DO:  The simple loop
    //
    //           forever
    //           begin
