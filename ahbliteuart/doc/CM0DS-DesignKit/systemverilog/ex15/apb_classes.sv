...
class APB_Driver;
  ...
  // harness module
  //
  virtual APB_test_intf.test_mp TB_intf;
  ...
  // the call to new(...)
  //
  function new(virtual APB_test_intf.test_mp TB_intf);
  ...
  task drive(APB_Trans t);
    ...
    TB_intf.cb.PENABLE <= 0 ;
    TB_intf.cb.PSEL    <= 1 ;
    TB_intf.cb.PADDR   <= t.addr ;

    if (t.dir == dir_Wr)
    begin // write
      TB_intf.cb.PWRITE    <= 1 ;
      TB_intf.cb.PWDATA    <= t.data ;
    end
    else
    begin // read
      TB_intf.cb.PWRITE    <= 0 ;
