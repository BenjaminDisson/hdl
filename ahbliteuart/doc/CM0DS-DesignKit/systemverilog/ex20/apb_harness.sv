...
interface APB_test_intf (input bit PCLK);
  ...
  // TO DO - add a clocking block for the monitor
  // The clocking block should be named "mon_cb" (to match the code in apb_mon.sv)
  // (all clockvars are input)
  clocking mon_cb @(posedge PCLK);
    input #1step PRDATA,PADDR, PWDATA, PSEL, PENABLE, PWRITE;
  endclocking : mon_cb
  ...
  // TO DO - add a modport to give access to the monitor clocking block
