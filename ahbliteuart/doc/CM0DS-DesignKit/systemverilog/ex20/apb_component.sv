...
class APB_Component extends Component;
  ...
  function new(string _name, Component _parent,
    ...
    // TO DO - create an instance of APB_Monitor,
    // pass the virtual interface as a constructor argument
    mon = new("my_monitor", this, _hook);

    // TO DO - create an instance of APB_Cov_Collector
    cov = new("my_collector", this, 1);

    stim_gen.sink = stim_chan;
    driver.source = stim_chan;

    // TO DO - connect the monitor and coverage collector using mon_chan
    mon.sink = mon_chan;
    cov.source = mon_chan;
