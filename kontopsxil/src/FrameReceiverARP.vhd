--   ----------------------------------------------------------------------------------------------
--   Nom du fichier    	: FrameReceiverArp.vhd  
...
--   ----------------------------------------------------------------------------------------------
--   21/09/2021	      : 1.0   : Benjamin Disson  	: Creation 
...
end FrameReceiverArp;

architecture  rtl  of  FrameReceiverArp  is
-------------------------------------------------------------------------------
...
--CONSTANT C_SRC_MAC : std_logic_vector(6*8-1 downto 0) := x"BD155014EAD5";
CONSTANT C_SRC_MAC : std_logic_vector(6*8-1 downto 0) := x"001234567890";
--CONSTANT C_DST_MAC : std_logic_vector(6*8-1 downto 0) := x"EA1ABEAFEAD5";
...
--  TYPE DECLARATION
-------------------------------------------------------------------------------
type   T_FRAME_RECEIVING is (WaitToBegin, ReceivePreamble, ReceiveDest, ReceiveSrc, ReceiveLenght, ReceiveARP,ReceiveIp, ReceiveUdp,ReceivePayload, ReceiveCRC); 
signal StateCurrent : T_FRAME_RECEIVING;
...
--signal s_LfsrCpt            : natural range C_NB_REG_PRBS downto 0; -- Nombre de bascules necessaire pour une prbs-16 bits
...
-- MAC
--signal s_MacSrc                : std_logic_vector(6*8-1 downto 0);
signal s_MacDst                : std_logic_vector(6*8-1 downto 0);
signal s_MacLenght             : std_logic_vector(2*8-1 downto 0);
...
signal s_IpSender			   : std_logic_vector(31 downto 0);
signal s_IpTarget			   : std_logic_vector(31 downto 0);
...
-- |     Hw addr lenght    |  logical addr lenght  |             Operation              |
-- |                                       Sender MAC addr (1)                          |
-- |               Sender MAC addr (2) = Src       |          Sender IP addr (1)        |
-- |               Sender IP addr (2)  = Src       |      Destinataire MAC addr (1)     |
-- |                                Destinataire MAC addr (2)                           |
-- |                                  Destinataire IP addr                              |
-- -------------------------------------------------------------------------------------
-- Example ARP request : Je suis l’hôte « 00 08 54 0b 21 77», Est-ce que l’hôte possédant l’adresse Ip 192.168.0.1 peut me retourner son adresse physique ? 
-- 1) MAC
-- ffffffffffff    0008540B2177       0806            
-- Broadcat        MAC Src            Prot. ARP       
--
-- 2) ARP on Eth
-- 0001             0800                 06              04                  0001     
-- ARP sur Eth      Protocol type        Hw addr lenght  Prot. Addr Length   Operation
--
-- 0008540B2177      c0a80003         000000000000   c0a80001        
...
-------------------------------------------------------------------------------
    ...
    begin
        if i_arst_n = '0' then
		    ...
		    if ((s_BitCounter = 0) and (i_RxEn = '1')) then
				s_BitCounter <= 1 ;
            elsif ((s_BitCounter = C_FRAME_MAX_SIZE) or (i_RxEn = '0')) then 
                s_BitCounter <= 0 ;
            else
    ...
    --       machine à état   
    -------------------------------------        
    ProcReceiveFSM : process (i_arst_n,i_sys_clk_tx)
    begin
        if (i_arst_n = '0') then
            ...
            case StateCurrent is
                when WaitToBegin =>
					if i_RxEn = '1' then						
						...
						--      | |_| |_| |_| |_| |_| |_| |_| |_| |_| |_| |_| |_
						--        __|_______________________|__________
						-- TX_en /  |                       |    
						--          ^                       ^
						-- TXD   P1  P2  P3  P4  P5  P6  P7  P8
						-- Count     1   2   3   4   5   6   7   8
						
						-- When TX_EN is "on" the first byte of the preambule is sent
				

						s_InitCrc    <= '1'; --TODO a gérer pour UDP et ARP
						s_RunCrc     <= '0'; --TODO a gérer pour UDP et ARP						
						s_DataForCrc <= (others =>'0'); --TODO a gérer pour UDP et ARP
						StateCurrent <= ReceivePreamble;						
                    end if;
                ...
                when ReceivePreamble => 
				-- [(s_BitCounter = 0) and (i_RxEn = '1')] => s_BitCounter <= 1 
				-- 7 octets -- OLD :8 octets moins 4 bits = 60 bits /4 = 15
                    s_InitCrc    <= '0';
                    s_RunCrc     <= '0'; 
                    s_DataForCrc <= (others =>'0');    
					-- ARP
                ...
                when ReceiveDest => 
				-- 6 octets : bit_counter = 14 				
                    s_InitCrc       <= '0';
                    s_RunCrc        <= '1';
                    s_DataForCrc(7) <= i_Rxd(7);
                    s_DataForCrc(6) <= i_Rxd(6);
                    s_DataForCrc(5) <= i_Rxd(5);
                    s_DataForCrc(4) <= i_Rxd(4);
                    s_DataForCrc(3) <= i_Rxd(3);
                    s_DataForCrc(2) <= i_Rxd(2);
                    s_DataForCrc(1) <= i_Rxd(1);
                    s_DataForCrc(0) <= i_Rxd(0);					
					s_MacDst((47-(s_BitCounter-8)*8) downto (40-(s_BitCounter-8)*8)) <= i_Rxd;
					-- ARP
					o_RunArp <='0';					
                    if s_BitCounter = 13 then 
                        StateCurrent <= ReceiveSrc;
                    else
						
						if (s_MacDst(47 downto 8) & i_Rxd = x"ffffffffffff") then	
							-- si on envoie en broadcast, je répond par ma valeur par défaut
							o_MacDst <= G_MAC_ADDR;
						else
							-- sinon je suis l'adresse de destination
							o_MacDst <= s_MacDst(47 downto 8) & i_Rxd;
						end if;						
				...
				-- 6 octets : bit_counter = 20 
                    ...
                    s_DataForCrc(1) <= i_Rxd(1);
                    s_DataForCrc(0) <= i_Rxd(0);
					o_MacSrc((47-(s_BitCounter-14)*8) downto (40-(s_BitCounter-14)*8)) <= i_Rxd;
					-- ARP
					o_RunArp <='0';
                    if s_BitCounter = 19 then
                        StateCurrent <= ReceiveLenght;
                    else
                        StateCurrent <= ReceiveSrc;
                    end if;        
                    
                when ReceiveLenght => -- 2 octets : bit_counter = 22 -- OLD 16/4 = 4    + 40 = 44
                    ...
                    s_DataForCrc(1) <= i_Rxd(1);
                    s_DataForCrc(0) <= i_Rxd(0);      					
					s_MacLenght((15-(s_BitCounter-20)*8) downto (8-(s_BitCounter-20)*8)) <= i_Rxd;
					-- ARP
					o_RunArp <='0';
-- Check ARP					
                    if s_BitCounter = 21 then
						if (s_MacLenght(15 downto 8) = x"08") and (i_Rxd = x"06") then
						--ARP frame
						StateCurrent <= ReceiveArp;
						elsif (s_MacLenght(15 downto 8) = x"80") and (i_Rxd = x"00") then
						--IP
                        StateCurrent <= ReceiveIp;
                ...
                when ReceiveArp =>				
					...
					if s_BitCounter < 29 then 
						-- TODO : Other check to finish
						o_RunArp <='0';
						StateCurrent <= ReceiveArp;
						
					elsif (s_BitCounter = 29 and (i_Rxd = x"01") ) then
						-- Check Request
						o_RunArp <='0';
						StateCurrent <= ReceiveArp;
						
					elsif s_BitCounter > 29 and s_BitCounter < 36 then
					-- Sender MAC addr : already done (o_MacSrc)
						o_RunArp <='0';
						StateCurrent <= ReceiveArp;
						
					elsif s_BitCounter > 35 and s_BitCounter < 40 then					
					-- Sender IP addr
						o_RunArp <='0';
						o_IpSender((31-(s_BitCounter-36)*8) downto (24-(s_BitCounter-36)*8)) <= i_Rxd;
						StateCurrent <= ReceiveArp;
						
					elsif s_BitCounter > 39 and s_BitCounter < 46 then
					-- Dest MAC addr : in reception = 000000000000 TODO : who care about 000000000000 ?
						o_RunArp <='0';
						StateCurrent <= ReceiveArp;
					
					elsif s_BitCounter > 45 and s_BitCounter < 50 then
					--Dest. IP addr
						o_RunArp <='0';
						s_IpTarget((31-(s_BitCounter-46)*8) downto (24-(s_BitCounter-46)*8)) <= i_Rxd;
						StateCurrent <= ReceiveArp;				
						
						
					elsif (s_BitCounter = 50) and (s_IpTarget = G_IP_ADDR) then
					-- Launch the ARP response
						o_IpTarget <= s_IpTarget;
						o_RunArp <='1';
						StateCurrent <= WaitToBegin;
