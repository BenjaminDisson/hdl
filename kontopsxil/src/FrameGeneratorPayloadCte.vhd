--   ----------------------------------------------------------------------------------------------
--   Nom du fichier    	: FrameGeneratorPayloadCte.vhd  
--   Auteur             : Benjamin Disson
--   Date de creation	: 21/09/2016
--   Version            : 1.0
--   Description        : Génération de données de remplissage pour une trame Ethernet utilisant une payload MAC constante
--   Sous module        : Aucun
--   Modifications    
--   ----------------------------------------------------------------------------------------------
...
--   13/01/2022         : 4.0   : Benjamin Disson   : RGMII - use a payload MAC constante frame from preambule to CRC
...
entity  FrameGeneratorPayloadCte  is
port ( 	
    -- Clock et Reset
...
end FrameGeneratorPayloadCte;

architecture  rtl  of  FrameGeneratorPayloadCte  is
-------------------------------------------------------------------------------
...
CONSTANT C_MAC_PAYLOAD : std_logic_vector(367 downto 0) := x"4500002EB3FE000080110540C0A8002CC0A8000404000400001A2DE8000102030405060708090A0B0C0D0E0F1011";
    ...
    begin
        if i_arst_n = '0' then
            s_BitCounter <= 0;
             ...
             if s_BitCounter = C_FRAME_MAX_SIZE then 
                s_BitCounter <= 1 ;
            else
        ...
        if (i_arst_n = '0') then
            ...
            case StateCurrent is
				...
				when SendPayload => -- 128 octets : bit_counter = 150 -- OLD 1024/4 = 256 + 44 = 300
                    o_Tx    <= C_MAC_PAYLOAD((367-((s_BitCounter-23)*8)) downto (360-((s_BitCounter-23)*8)));
                    o_TxEn  <= '1';
                    s_InitCrc <= '0';
                    s_RunCrc  <= '1'; 
					s_DataForCrc(7) <= C_MAC_PAYLOAD((360-((s_BitCounter-23)*8)));
					s_DataForCrc(6) <= C_MAC_PAYLOAD((361-((s_BitCounter-23)*8)));
					s_DataForCrc(5) <= C_MAC_PAYLOAD((362-((s_BitCounter-23)*8)));
					s_DataForCrc(4) <= C_MAC_PAYLOAD((363-((s_BitCounter-23)*8)));
					s_DataForCrc(3) <= C_MAC_PAYLOAD((364-((s_BitCounter-23)*8)));
					s_DataForCrc(2) <= C_MAC_PAYLOAD((365-((s_BitCounter-23)*8)));
					s_DataForCrc(1) <= C_MAC_PAYLOAD((366-((s_BitCounter-23)*8)));
					s_DataForCrc(0) <= C_MAC_PAYLOAD((367-((s_BitCounter-23)*8)));      					
